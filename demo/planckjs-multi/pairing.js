            ...
            if (ok) {
                ...
                if (event.peers[0].uuid === myUUID) {
                    ...
                    setTimeout(() => {
                        ...
                        mesh.broadcastAndListen(message.buffer).forEach(promise => promise.then(reply => {
                            peersConfirmed++;
                            if (peersConfirmed === event.peers.length - 1) {
                                waiting.style.display = 'none';
                                gameArea.style.display = 'flex';
                                startGame(event.peers, mesh, reply.sourceTimestamp + deltaTimeToStart * 1000);
                            }
                        }));
                    }, 5000);
                }
...
mesh.messageEmitter.addEventListener((uuid, message) => {
    ...
    if (info[0] === 33) { // 33 == delta time proposal
        // send the reply
        ...
        waiting.style.display = 'none';
        gameArea.style.display = 'flex';
        startGame(peers, mesh, message.timestampToLocalTime + deltaTimeToStart * 1000);
    }
});
